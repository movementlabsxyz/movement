...
async fn main() -> Result<(), anyhow::Error> {
	...
	let (hsm, public_key) = match provider.as_str() {
		"AWS" => {
			let aws_kms_hsm = hsm::aws_kms::AwsKms::<Secp256k1>::try_from_env()
				.await?
				.create_key()
				.await?
		...
		"VAULT" => {
			let vault_hsm = hsm::hashi_corp_vault::HashiCorpVault::<Ed25519>::try_from_env()?
				.create_key()
				.await?
				.fill_with_public_key()
				.await?;
	...
	let server_task = task::spawn(async move {
		...
		Server::bind(&addr).serve(app.into_make_service()).await.expect("Server failed");
	});
