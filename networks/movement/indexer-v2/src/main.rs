...
fn main() -> Result<(), anyhow::Error> {
	...
	let runnable_processor_config: IndexerProcessorConfig = maptos_config.indexer_processor_v2.clone().into();
	...
	runtime.block_on(async move {
		...
		set.spawn(async move {
			...
			start_metrics_server(maptos_config.indexer_processor_v2.metrics_config.listen_hostname, maptos_config.indexer_processor_v2.metrics_config.listen_port).await
		});
		set.spawn(async move {
			tracing::info!("Starting health server");
			run_service(maptos_config.indexer_processor_v2.health_config.hostname, maptos_config.indexer_processor_v2.health_config.port).await
		});
		set.spawn(async move {
