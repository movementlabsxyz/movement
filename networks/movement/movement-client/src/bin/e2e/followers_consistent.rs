...
pub fn get_node_url(config: &movement_config::Config) -> Result<Url, anyhow::Error> {
...
pub fn get_faucet_url(config: &movement_config::Config) -> Result<Url, anyhow::Error> {
...
) -> Result<(DotMovement, movement_config::Config, Client, FaucetClient), anyhow::Error> {
	let follower_dot_movement = follower_index_to_dot_movement(follower_index, lead_dot_movement)?;

	let config = get_movement_config(&follower_dot_movement)?;
