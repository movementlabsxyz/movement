...
impl ElsaToBiarritzRc1 {
	pub async fn execute(&self) -> Result<(), anyhow::Error> {
		...
		let dot_movement =
			self.movement_args.dot_movement().context("failed to get dot movement")?;

		// run the migration
		let config = dot_movement
			.migrate_elsa_to_biarritz_rc1()
			.await
			.context("failed to migrate config from Elsa to Biarritz RC1")?;
