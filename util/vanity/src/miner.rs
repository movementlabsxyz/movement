use aptos_sdk::types::{account_address::{AccountAddress, create_resource_address}, LocalAccount};
use rand::thread_rng;
use rand::Rng;
use rayon::prelude::*;
...
fn matches(addr: &AccountAddress, start: &str, end: &str) -> bool {
    ...
    (start.is_empty() || addr_hex.starts_with(start)) &&
    (end.is_empty() || addr_hex.ends_with(end))
}
