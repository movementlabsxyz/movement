...
use prometheus::{Encoder, Counter, Gauge};
...
impl ConsensusMetrics {
    fn new() -> Self {
        Self {
            block_height: Gauge::new("movement_consensus_last_committed_version", "Last committed version").unwrap(),
            round: Gauge::new("movement_consensus_current_round", "Current consensus round").unwrap(),
            committed_txns: Counter::new("movement_state_sync_version", "Synced version").unwrap(),
            round_timing: Gauge::new("movement_consensus_round_timing", "Consensus round timing").unwrap(),
        }
    }
}
...
impl DbMetrics {
    fn new() -> Self {
        Self {
            db_size: Gauge::new("movement_storage_ledger_version", "Ledger version").unwrap(),
        }
    }
}
...
impl MempoolMetrics {
    fn new() -> Self {
        Self {
            ...
            processing_time: Gauge::new("movement_mempool_txn_processing_time", "Transaction processing time").unwrap(),
            broadcast_time: Gauge::new("movement_mempool_broadcast_time", "Transaction broadcast time").unwrap(),
            pending_txns: Gauge::new("movement_mempool_pending_txns", "Pending transactions count").unwrap(),
        }
    }
}
...
impl TransactionMetrics {
    fn new() -> Self {
        Self {
            retrieval_latency: Gauge::new("movement_txn_retrieval_latency", "Transaction retrieval latency").unwrap(),
            commit_latency: Gauge::new("movement_txn_commit_latency", "Transaction commit latency").unwrap(),
            save_latency: Gauge::new("movement_txn_save_latency", "Transaction save latency").unwrap(),
        }
    }
}
