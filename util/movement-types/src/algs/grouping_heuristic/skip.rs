...
impl <T> Skip<T> {
    ...
    pub fn skip(&mut self, distribution: Vec<GroupingOutcome<T>>) -> Result<Vec<GroupingOutcome<T>>, anyhow::Error> {

        Ok(distribution)

    }

    pub fn apply(&mut self, distribution: Vec<GroupingOutcome<T>>) -> Result<Vec<GroupingOutcome<T>>, anyhow::Error> {
...
impl <T> SkipFor<T> {
    ...
    pub fn new(count : usize, heuristic: Box<dyn GroupingHeuristic<T>>) -> Self {
        SkipFor {
            skip_if: SkipIf::new(false, heuristic),
            counter: 0,
            for_count: count
        }
    }
