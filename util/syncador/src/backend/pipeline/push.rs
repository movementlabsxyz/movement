...
pub struct Pipeline {
	pub backends: Vec<Box<dyn PushOperations + Sync>>,
}

impl Pipeline {
	pub fn new(backends: Vec<Box<dyn PushOperations + Sync>>) -> Self {
		Self { backends }
	}

	pub fn boxed(backends: Vec<Box<dyn PushOperations + Sync>>) -> Box<Self> {
