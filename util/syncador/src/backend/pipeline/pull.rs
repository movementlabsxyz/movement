...
pub struct Pipeline {
	pub backends: Vec<Box<dyn PullOperations + Sync>>,
}

impl Pipeline {
	pub fn new(backends: Vec<Box<dyn PullOperations + Sync>>) -> Self {
		Self { backends }
	}

	pub fn boxed(backends: Vec<Box<dyn PullOperations + Sync>>) -> Box<Self> {
