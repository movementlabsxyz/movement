...
use rustix::{fd::AsFd, fs::FlockOperation};
use thiserror::Error;
...
impl<T: AsFd> FileRwLock<T> {
	...
	pub async fn try_write(&self) -> Result<FileRwLockWriteGuard<'_, T>, FileRwLockError> {
		let (res, write) = {
			...
			(flock(&*file, FlockOperation::NonBlockingLockExclusive).await, file)
		};

		match res {
			Ok(_) => Ok(FileRwLockWriteGuard { guard: write }),
			Err(e) => Err(e.into()),
		}
	}
	...
	pub async fn try_read(&self) -> Result<FileRwLockReadGuard<'_, T>, FileRwLockError> {
		let (res, read) = {
			...
			(flock(&*file, FlockOperation::NonBlockingLockShared).await, file)
		};

		match res {
			Ok(_) => Ok(FileRwLockReadGuard { guard: read }),
			Err(e) => Err(e.into()),
		}
	}
