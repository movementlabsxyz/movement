module moveth::moveth_tests{
    ...
    fun test_basic_flow(creator: &signer, minter: &signer, master_minter: &signer, denylister: &signer) {
        // create a resource account from the creator, mocking the module publishing process
        create_account_for_test(signer::address_of(creator));
        resource_account::create_resource_account(creator, vector::empty(), vector::empty());
        moveth::init_for_test(master_minter);
