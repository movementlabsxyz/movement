		...
		A: Into<Vec<u8>> + std::clone::Clone,
	{
		...
		let (action_kind, chain_id) = match event.contract_event {
			...
			BridgeContractEvent::Cancelled(_) => {
				//TODO
				tracing::error!("Receive a Cancelled event. Not implemented, not processed");
				(TransferActionType::NoAction, state.init_chain)
			}
			BridgeContractEvent::Refunded(_) => {
				//TODO
				tracing::error!("Receive a Refunded event. Not implemented, not processed");
				(TransferActionType::NoAction, state.init_chain)
			}
		};
