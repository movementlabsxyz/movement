...
use bridge_service::chains::ethereum::{event_monitoring::EthMonitoring, types::EthAddress};
...
async fn test_eth_client_initiator_complete_transfer() {
	...
	let res = eth_client_harness
		.eth_client
		.initiator_complete_bridge_transfer(bridge_transfer_id, hash_lock_pre_image)
		.await;
...
async fn test_eth_client_refund_transfer() {
	...
	// let rpc_provider = eth_client_harness.rpc_provider().await;

	// let init_contract = bridge_service::chains::ethereum::types::AtomicBridgeInitiatorMOVE::new(
	// 	Address::from_str(&config.eth.eth_initiator_contract).unwrap(),
	// 	rpc_provider.clone(),
	// );

	// Approve the ETH initiator contract to spend Amount of MOVE
	// let approve_call = init_contract
	// 	.refundBridgeTransfer(alloy::primitives::FixedBytes([2; 32]))
	// 	.from(eth_client_harness.signer_address());
	// approve_call.send().await.unwrap().get_receipt().await.unwrap();
	...
	// Wait end of timelock
	tokio::time::sleep(tokio::time::Duration::from_secs(20)).await;

	let res = eth_client_harness.eth_client.refund_bridge_transfer(bridge_transfer_id).await;
