...
impl Executor {
	...
	pub fn get_ledger_info_with_sigs(
		...
		let block_info = BlockInfo::new(
			1, 0, block_id, root_hash, version, 0, /* timestamp_usecs, doesn't matter */
			None,
		);
	...
	pub async fn execute_block(&self, block: ExecutableBlock) -> Result<(), anyhow::Error> {
		...
		{
			// let reader = self.db.read().await.reader.clone();
			// let proof = reader.get_state_proof(state_compute.version())?;
		}
