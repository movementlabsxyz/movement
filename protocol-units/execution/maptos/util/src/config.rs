    ...
    impl Config {
        ...
        pub fn try_from_env() -> Result<Self, anyhow::Error> {
            ...
            let chain_id = match std::env::var(Self::CHAIN_ID_ENV_VAR) {
                Ok(chain_id) => serde_json::from_str(chain_id.as_str()).context(
        ...
        pub fn write_to_env(&self) -> Result<(), anyhow::Error> {
            ...
            std::env::set_var(Self::APTOS_REST_LISTEN_ADDR_ENV_VAR, self.aptos_rest_listen_url.clone());
            std::env::set_var(Self::APTOS_FAUCET_LISTEN_ADDR_ENV_VAR, self.aptos_faucet_listen_url.clone());
        ...
        pub fn write_bash_export_string(&self) -> Result<String, anyhow::Error> {
            Ok(format!(
                "export {}={}\nexport {}={}\nexport {}={}\nexport {}={}\nexport {}={}",
                Self::CHAIN_ID_ENV_VAR, serde_json::to_string(&self.chain_id)?,
                Self::APTOS_REST_LISTEN_ADDR_ENV_VAR, self.aptos_rest_listen_url,
                Self::APTOS_FAUCET_LISTEN_ADDR_ENV_VAR, self.aptos_faucet_listen_url,
                Self::APTOS_PRIVATE_KEY_ENV_VAR, self.aptos_private_key.to_encoded_string()?,
                Self::APTOS_PUBLIC_KEY_ENV_VAR, self.aptos_public_key.to_encoded_string()?
            ))
        }
