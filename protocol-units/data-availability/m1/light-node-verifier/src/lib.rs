...
pub trait Verifier {
    ...
    async fn verify(
        ...
        match verification_mode {
            VerificationMode::Cowboy => {
                self.verify_cowboy(verification_mode, blob, height).await
            },
            VerificationMode::ValidatorIn => {
                self.verifiy_validator_in(verification_mode, blob, height).await
            },
            VerificationMode::MOfN => {
                self.verify_m_of_n(verification_mode, blob, height).await
            },
        }
    }
