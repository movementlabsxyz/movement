...
impl Verifier for V1Verifier {
    ...
    async fn verify(
        ...
        // get the proof
        let proofs = self.client.blob_get_proof(height, self.namespace.clone(), celestia_blob.commitment).await?;
        ...
        for proof in proofs.iter() {
            ...
            proof.verify_complete_namespace(&root_hash, &leaves, self.namespace.into()).map_err(
                |e| anyhow::anyhow!("Failed to verify proof: {:?}", e)
            )?;
...
pub mod test {
    ...
    pub async fn test_valid_verifies() -> Result<(), anyhow::Error> {
        ...
        let blob = Blob::new(
            config.celestia_namespace.clone(),
            data.clone(),
        )?;
    ...
    pub async fn test_wrong_height_does_not_verify() -> Result<(), anyhow::Error> {
        ...
        let wrong_height_included = verifier.verify(VerificationMode::Cowboy, &data, height+1).await;
