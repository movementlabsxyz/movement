...
impl Deploy {
	pub async fn setup(
		...
		let mut well_known_account_private_keys = if let Some(existing_testing_config) = config.testing.clone() {
		...
		let testing_config = common::testing::Config {
			...
			mcr_testing_admin_account_private_key: deploy.mcr_deployment_account_private_key.clone(),
