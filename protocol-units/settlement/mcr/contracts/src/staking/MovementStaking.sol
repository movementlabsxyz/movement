...
contract MovementStaking is MovementStakingStorage, IMovementStaking, BaseStaking {
    ...
    function registerDomain(uint256 epochDuration, address[] calldata custodians) external {
    ...
    function _addStake(
        ...
        epochStakesByDomain[domain][epoch][custodian][attester][delegator] += amount;
    ...
    function _addUnstake(
        ...
        epochUnstakesByDomain[domain][epoch][custodian][attester][delegator] += amount;
    }
    ...
    function getStakeAtEpoch(address domain, uint256 epoch, address custodian, address attester, address delegator)
        public
        view
        returns (uint256)
    {
        return epochStakesByDomain[domain][epoch][custodian][attester][delegator];
    }

    // get all attester weight for a given epoch
    function getAllStakeAtEpoch(address domain, uint256 epoch, address custodian, address attester)
        public
        view
        returns (uint256)
    {
        uint256 totalStake = 0;
        for (uint256 i = 0; i < delegatorsByAttesterByDomain[domain][attester].length(); i++) {
            address delegator = delegatorsByAttesterByDomain[domain][attester].at(i);
            totalStake += getStakeAtEpoch(domain, epoch, custodian, attester, delegator);
        }
        return totalStake;
    }

    // gets the stake for a given attester at the current epoch
    function getAllCurrentEpochStake(address domain, address custodian, address attester)
        public
        view
        returns (uint256)
    {
        return getAllStakeAtEpoch(domain, getCurrentEpoch(domain), custodian, attester);
    }

    // gets all the attester stakes for the current epoch
    function getCurrentEpochStake(address domain, address custodian, address attester, address delegator)
        public
        view
        returns (uint256)
    {
        return getStakeAtEpoch(domain, getCurrentEpoch(domain), custodian, attester, delegator);
    }

    // gets the unstake for a given attester at a given epoch
    function getUnstakeAtEpoch(address domain, uint256 epoch, address custodian, address attester, address delegator)
        public
        view
        returns (uint256)
    {
        return epochUnstakesByDomain[domain][epoch][custodian][attester][delegator];
    }

    // gets all the stake a given attester is unstaking at a given epoch
    function getAllUnstakeAtEpoch(address domain, uint256 epoch, address custodian, address attester)
        public
        view
        returns (uint256)
    {
    ...
    function getCurrentEpochUnstake(address domain, address custodian, address attester, address delegator)
        public
        view
        returns (uint256)
    {
        return getUnstakeAtEpoch(domain, getCurrentEpoch(domain), custodian, attester, delegator);
    }

    // gets all the stake an attester is unstaking for the current epoch
    function getAllCurrentEpochUnstakeWeight(address domain, address custodian, address attester)
        public
        view
        returns (uint256)
    {
        return getAllUnstakeAtEpoch(domain, getCurrentEpoch(domain), custodian, attester);
    }

    // gets the total stake for a given epoch
    function getTotalStakeForEpoch(address domain, uint256 epoch, address custodian) public view returns (uint256) {
    ...
    function stakeWithDelegate(address domain, IERC20 custodian, uint256 amount, address delegatee) external {
        _stakeWithDelegate(domain, custodian, amount, delegatee);
    }

    // stakes for the next epoch
    function _stakeWithDelegate(address domain, IERC20 custodian, uint256 amount, address delegatee)
        internal
        onlyRole(WHITELIST_ROLE)
    {
        ...
        emit AttesterStaked(domain, getNextEpoch(domain), address(custodian), msg.sender, delegatee, amount);
    }
    ...
    function unstakeWithDelegate(address domain, address custodian, uint256 amount, address delegatee) external {
        _unstakeWithDelegate(domain, custodian, amount, delegatee);
    }

    // unstakes an amount for the next epoch
    function _unstakeWithDelegate(address domain, address custodian, uint256 amount, address delegatee)
        internal
        onlyRole(WHITELIST_ROLE)
    {
        ...
        _addUnstake(domain, getNextEpochByBlockTime(domain), custodian, delegatee, msg.sender, amount);

        emit AttesterUnstaked(domain, getNextEpoch(domain), custodian, msg.sender, delegatee, amount);
    }

    // rolls over the stake and unstake for a given attester
    function _rollOverAttester(address domain, uint256 epochNumber, address custodian, address attester) internal {
        ...
        for (uint256 i = 0; i < attestersByDelegatorByDomain[domain][attester].length(); i++) {
            ...
            uint256 unstakeAmount = getUnstakeAtEpoch(domain, epochNumber + 1, custodian, delegatee, attester);
            if (unstakeAmount > stakeAmount) {
                unstakeAmount = stakeAmount;
            }
            uint256 remainder = stakeAmount - unstakeAmount;

            _addStake(domain, epochNumber + 1, custodian, delegatee, attester, remainder);
            ...
            emit AttesterEpochRolledOver(attester, delegatee, epochNumber, custodian, stakeAmount, unstakeAmount);
        }
    }
    ...
    function _slashStake(
        ...
        uint256 stakeForEpoch = getStakeAtEpoch(domain, targetEpoch, custodian, delegator, attester);

        // deduct the amount from the attester's stake, account for underflow
        if (stakeForEpoch < amount) {
            _removeStake(domain, targetEpoch, custodian, attester, delegator, stakeForEpoch);
        } else {
            _removeStake(domain, targetEpoch, custodian, attester, delegator, amount);
        }
    }
     ...
     * @param attester The attester to slash
     */
    function _slashUnstake(address domain, uint256 epoch, address custodian, address attester, address delegator)
        internal
    {
        // unstake slash will always target the next epoch
        uint256 stakeForEpoch = getStakeAtEpoch(domain, epoch, custodian, attester, delegator);
        uint256 targetEpoch = epoch + 1;
        uint256 unstakeForEpoch = getUnstakeAtEpoch(domain, targetEpoch, custodian, attester, delegator);
        ...
        if (unstakeForEpoch > stakeForEpoch) {
            ...
            _setUnstake(domain, targetEpoch, custodian, attester, delegator, stakeForEpoch);
        }
    }
    ...
    function slash(
        ...
        for (uint256 i = 0; i < attesters.length; i++) {
            ...
            uint256 refundAmount = Math.min(
                getStakeAtEpoch(msg.sender, getCurrentEpoch(attesters[i]), custodians[i], attesters[i], delegators[i]),
            ...
            _slashStake(msg.sender, getCurrentEpoch(msg.sender), custodians[i], attesters[i], delegators[i], amounts[i]);

            _slashUnstake(msg.sender, getCurrentEpoch(msg.sender), custodians[i], attesters[i], attesters[i]);
        }
    }

    function _payAttester(address from, address attester, address custodian, uint256 amount) internal {
    ...
    function reward(address[] calldata attesters, uint256[] calldata amounts, address[] calldata custodians) public {
