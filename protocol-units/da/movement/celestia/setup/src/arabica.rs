...
impl Arabica {
	...
	pub async fn create_and_fund_account(
		 ...
		 */
		let celestia_address = config
			.appd
			.celestia_validator_address
			.context("Celestia validator address is not set in the config.")?
			.clone();
		...
		while retry_count < max_retries {
			...
			let tx_hash =
				res.get("txHash").context("Failed to get the txHash field from the response.")?;
