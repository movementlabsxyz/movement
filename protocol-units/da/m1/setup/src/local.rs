...
impl Local {
	...
	async fn make_dirs(&self, config: Config) -> Result<Config, anyhow::Error> {
	...
	async fn setup_celestia(
		...
		let config = self.initialize_memseq_config(dot_movement, config).await?;
		let mut config = self.make_dirs(config).await?;
