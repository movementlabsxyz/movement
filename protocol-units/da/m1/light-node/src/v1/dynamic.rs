...
impl LightNodeV1Operations for LightNodeV1 {
    ...
    async fn try_from_env() -> Result<Self, anyhow::Error> {
        ...
        let which = std::env::var("M1_DA_LIGHT_NODE_MODE").unwrap_or_else(|_| "passthrough".to_string());

        match which.as_str() {
            "passthrough" => {
                Ok(Self::PassThrough(passthrough::LightNodeV1::try_from_env().await?))
            },
            "sequencer" => {
                Ok(Self::Sequencer(sequencer::LightNodeV1::try_from_env().await?))
            },
            _ => {
                Err(anyhow::anyhow!("Unknown mode: {}", which))
            }
        }
    ...
    async fn run_background_tasks(&self) -> Result<(), anyhow::Error> {
        ...
        match self {
            Self::PassThrough(pass_through) => {
                pass_through.run_background_tasks().await
            },
            Self::Sequencer(sequencer) => {
                sequencer.run_background_tasks().await
            }
        }
...
impl LightNodeService for LightNodeV1 {
    ...
    > {
        ...
        match self {
            Self::PassThrough(pass_through) => {
                pass_through.stream_read_from_height(request).await
            },
            Self::Sequencer(sequencer) => {
                sequencer.stream_read_from_height(request).await
            }
        }    
    ...
    > {
        ...
        match self {
            Self::PassThrough(pass_through) => {
                pass_through.stream_read_latest(request).await
            },
            Self::Sequencer(sequencer) => {
                sequencer.stream_read_latest(request).await
            }
        }
    ...
    match self {
            Self::PassThrough(pass_through) => {
                pass_through.read_at_height(request).await
            },
            Self::Sequencer(sequencer) => {
                sequencer.read_at_height(request).await
            }
        }
    ...
    > {
        ...
        match self {
            Self::PassThrough(pass_through) => {
                pass_through.batch_read(request).await
            },
            Self::Sequencer(sequencer) => {
                sequencer.batch_read(request).await
            }
        }
    ...
    > {
        ...
        match self {
            Self::PassThrough(pass_through) => {
                pass_through.batch_write(request).await
            },
            Self::Sequencer(sequencer) => {
                sequencer.batch_write(request).await
            }
        }
    ...
    match self {
            ...
            Self::Sequencer(sequencer) => {
                sequencer.update_verification_parameters(request).await
            }
        }
