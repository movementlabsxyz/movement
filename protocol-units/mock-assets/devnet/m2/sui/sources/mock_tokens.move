...
module mock_tokens::moveth {
  use sui::coin;
  use sui::url::new_unsafe_from_bytes;

  public struct MOVETH has drop {}

  fun init(witness: MOVETH, ctx: &mut TxContext) {
      let (treasury_cap, metadata) = coin::create_currency<MOVETH>(
            witness, 
            9, 
            b"MOVETH",
            b"MovETH", 
            b"Bridged Ethereum on Move", 
